-- Match net scoring: apply handicap strokes from (team A combined − team B combined) only,
-- spread across holes on the higher-handicap team—do not allocate full team totals separately.
...
BEGIN
  ...
  FOR h IN 1..9 LOOP
    ...
    IF hdiff > 0 THEN
      str_a := nhgl._strokes_received_on_hole(v_course_id, hdiff, v_actual, v_which);
      str_b := 0;
    ELSIF hdiff < 0 THEN
      str_a := 0;
      str_b := nhgl._strokes_received_on_hole(v_course_id, -hdiff, v_actual, v_which);
    ELSE
      str_a := 0;
      str_b := 0;
    END IF;
