-- Track who a substitute is replacing in per-player rounds.

ALTER TABLE nhgl.player_rounds
  ADD COLUMN IF NOT EXISTS subbing_for_player_id uuid REFERENCES nhgl.players (id) ON DELETE SET NULL;

COMMENT ON COLUMN nhgl.player_rounds.subbing_for_player_id IS 'If this round was submitted by a substitute, which roster player they replaced.';

DROP FUNCTION IF EXISTS nhgl.submit_player_round(uuid, uuid, uuid, uuid, boolean, jsonb, text, text);
